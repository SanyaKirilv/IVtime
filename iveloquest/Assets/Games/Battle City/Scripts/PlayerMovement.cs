...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void CalculateAxis()
    {
        if (player == 1)
        {
            if (joystick.Horizontal > 0.2f) axis_x = 1;
            else if (joystick.Horizontal < -0.2f) axis_x = -1;
            else axis_x = 0;
            if (joystick.Vertical > 0.2f) axis_y = 1;
            else if (joystick.Vertical < -0.2f) axis_y = -1;
            else axis_y = 0;
        }
    }
    ...
    public void ResetPosition()
    {
        if (player == 1)
        {
            trans.position = new Vector3(-4, -21, 0);
        }
    }
}
